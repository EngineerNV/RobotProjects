...
def changePWMvalues(newLeftValue, newRightValue):
    # Write the command to the microcontroller
...
def readResponse(serialPort):
    ...
    if msgType[0] == SENSE_ROBOT_COMMAND:
        ...
        result = struct.unpack_from('<HHH', msg)

        return result, msgType

    #else if msgType[0] == CHANGE_SENSE_THRESHOLD_COMMAND:
        #msg = serialPort.read(15)
        #result = struct.unpack_from('<HHH', msg)
    else:
        msg = serialPort.read(15)
...
#getSensors(serialPort)
#result, _ = readResponse(serialPort)
#print(result)
#serialPort.close()
