...
goalie_byte = 0x51 #hear the goalie scream
...
class Bail(object):
    ...
    def bail_out(self):
        if self.mode == "attacker":
            ...
            print("Goalie: The goalie has screamed into the void :)")

            # Attacker sends 3 bytes: start_byte, bail_byte, stop_byte
            # command to bail is int(86)
            packet = bytearray([int(start_byte), int(goalie_byte), int(stop_byte)])
            port.write(packet)
