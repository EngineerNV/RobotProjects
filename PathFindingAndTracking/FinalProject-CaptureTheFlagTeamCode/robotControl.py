...
def moveRobotXY(x, y, serialPort):
...
# Returns a named tuple in the order (distance, angle, time)
def moveRobotBackX(x, serialPort):
    # Write the command to the microcontroller
...
def changeSensorThreshold(serialPort, newThresholdLeft, newThresholdFront, newThresholdRight):
    # Write the command to the microcontroller
    serialPort.write(struct.pack('>BBBBBBB',CHANGE_SENSE_THRESHOLD_COMMAND,newThresholdLeft&0xFF,(newThresholdLeft>>8)&0xff,newThresholdFront&0xFF,(newThresholdFront>>8)&0xff, newThresholdRight&0xFF,(newThresholdRight>>8)&0xff))
